...
use lyra_primitives::{AccountId, Balance};
use quark_runtime::constants::currency::*;
use quark_runtime::{
    get_all_module_accounts, opaque::SessionKeys, BalancesConfig, ContractsConfig,
    CouncilMembershipConfig, GenesisConfig, SessionConfig, Signature, SudoConfig, SystemConfig,
    TechnicalMembershipConfig, ValidatorSetConfig, VestingConfig, WASM_BINARY,
};
...
pub type QuarkChainSpec = sc_service::GenericChainSpec<GenesisConfig>;
...
pub fn quark_config() -> Result<QuarkChainSpec, String> {
    QuarkChainSpec::from_json_bytes(&include_bytes!("../../res/quark.json")[..])
}

pub fn staging_config() -> Result<QuarkChainSpec, String> {
    ...
    Ok(QuarkChainSpec::from_genesis(
        // Name
        "Asterism Staging network",
        // ID
        "quark_staging",
        ChainType::Live,
        move || {
...
pub fn quark_development_config() -> Result<QuarkChainSpec, String> {
...
pub fn quark_local_testnet_config() -> Result<QuarkChainSpec, String> {
    let wasm_binary = WASM_BINARY.ok_or("Development wasm binary not available".to_string())?;
