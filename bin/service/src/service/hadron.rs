...
pub use crate::chain_spec::HadronChainSpec;
use asterism_rpc::hadron::FullDeps;
pub use hadron_runtime;
...
use hadron_runtime::opaque::Block;
use sc_telemetry::TelemetrySpan;

// Our native executor instance.
native_executor_instance!(
    pub HadronExecutor,
    hadron_runtime::api::dispatch,
    hadron_runtime::native_version,
...
where
    ...
    RuntimeApi::RuntimeApi:
        RuntimeEvmApiCollection<StateBackend = StateBackendFor<FullBackend, Block>>,
{
        ...
        RuntimeEvmApiCollection<StateBackend = StateBackendFor<FullBackend, Block>>,
{
    ...
    let rpc_extensions_builder = {
        ...
        Box::new(move |deny_unsafe, _| {
            ...
            asterism_rpc::hadron::create_full(deps, subscription_task_executor.clone())
        })
    };
        ...
        RuntimeEvmApiCollection<StateBackend = StateBackendFor<LightBackend, Block>>,
{
    ...
    let light_deps = asterism_rpc::hadron::LightDeps {
    ...
    let rpc_extensions = asterism_rpc::hadron::create_light(light_deps);
...
/// Create a new Asterism service for a full node.
#[cfg(feature = "full-node")]
pub fn hadron_new_full(
    config: Configuration,
) -> Result<
    (
        TaskManager,
        Arc<impl AsterismClient<Block, FullBackend, hadron_runtime::RuntimeApi>>,
    ),
    ServiceError,
> {
    let (components, client) =
        new_full::<hadron_runtime::RuntimeApi, HadronExecutor>(config, false)?;

    Ok((components, client))
}

/// Create a new Asterism service for a light client.
pub fn hadron_new_light(config: Configuration) -> Result<TaskManager, ServiceError> {
    new_light::<hadron_runtime::RuntimeApi, HadronExecutor>(config)
}